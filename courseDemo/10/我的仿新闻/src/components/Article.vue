<template>
    <div>
		...
		<div class="content">
			<div class="header clear"><h2><img v-bind:src="articlaData.author_face" alt=""/></h2><p>智能社</p></div>
		    <div class="cont">
		    	<h3>{{articlaData.title}}</h3>
		        <div class="time"><p>{{articlaData.time}}<span><img src="../assets/img/zan.png" alt=""/></span></p></div>
		        <div class="text-box" v-html="articlaData.content"></div>
		    </div>
		</div>
		<div class="foot-btn">
			<ul>
...
export default {
  mounted() {
    //当喔点击home 组件里面的路由时，跳转到article 组件，
    //   获取路由参数id,传入 fetchData 方法中
  ...
  methods: {
    fetchData(id) {
      var _this = this;
      axios.get("/src/data/article.data").then(function(res) {
          // console.log("res-article", res);
          _this.articlaData = res.data[id-1];
          //  console.log(" _this.articlaData", res.data[id-1]);
        }).catch(function(err) {
...
//  scoped 属性表明style里面的css只适用于当前组件元素，
//  同时也会影响设置了scoped属性的子组件的style样式
<style scoped>
